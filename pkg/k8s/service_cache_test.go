...
func (s *K8sSuite) TestServiceMerging(c *check.C) {
	...
	addresses := svcCache.GetIPsOfService(k8sSvcID)
	c.Assert(addresses, checker.DeepEquals, []*loadbalancer.L3n4Addr{loadbalancer.NewL3n4Addr(loadbalancer.TCP, net.ParseIP("127.0.0.1"), 80)})
}
